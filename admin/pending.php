...
$scholarshipTypes = $pdo->query("SELECT DISTINCT s.scholarship_type FROM scholarship s INNER JOIN user_application a ON s.scholarship_id = a.scholarship_id WHERE a.approved = 0 ORDER BY s.scholarship_type")->fetchAll(PDO::FETCH_COLUMN);
$barangays = $pdo->query("SELECT DISTINCT barangay FROM user_application WHERE approved = 0 AND barangay IS NOT NULL ORDER BY barangay")->fetchAll(PDO::FETCH_COLUMN);

// Get total pending applications count
$totalPending = $pdo->query("SELECT COUNT(*) FROM user_application WHERE approved = 0")->fetchColumn();
?>
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title"><i class="fas fa-clock mr-2"></i>Submitted Applications</h3>
              </div>
              ...
              <div class="card-body">
                ...
                <div class="results-count">
                  ...
                  <strong>Total Submitted Applications:</strong> <?= $totalPending ?> records
                </div>
                ...
                <div class="table-responsive">
                  <table id="applicationTable" class="table table-hover display">
                    ...
                    <tbody>
                      <?php
                      $qry = $pdo->query("SELECT a.application_id, concat(a.b_fname, ' ', a.b_lname) as b_name,  s.scholarship_type, s.amount, a.barangay, a.school_name, a.year_level, a.school_year, a.semester FROM user_application as a inner join scholarship as s on s.scholarship_id = a.scholarship_id WHERE a.approved = 0;");
...
$(document).ready(function(){
  ...
  var table = $('#applicationTable').DataTable({
    ...
    order: [[1, 'asc']],
    language: {
      search: "Search submitted applications:",
      lengthMenu: "Show _MENU_ applications",
      info: "Showing _START_ to _END_ of _TOTAL_ submitted applications",
      infoEmpty: "No submitted applications available",
