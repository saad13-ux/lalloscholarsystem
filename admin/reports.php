...
$totalBeneficiaries = $pdo->query("SELECT COUNT(*) FROM user_application WHERE approved = 1")->fetchColumn();
$totalAmount = $pdo->query("SELECT SUM(s.amount) FROM user_application as a INNER JOIN scholarship as s ON s.scholarship_id = a.scholarship_id WHERE a.approved = 1")->fetchColumn();
$uniqueBarangays = $pdo->query("SELECT COUNT(DISTINCT barangay) FROM user_application WHERE approved = 1")->fetchColumn();

// Get all unique values for filters
$scholarshipTypes = $pdo->query("SELECT DISTINCT s.scholarship_type FROM scholarship s INNER JOIN user_application a ON s.scholarship_id = a.scholarship_id WHERE a.approved = 1 ORDER BY s.scholarship_type")->fetchAll(PDO::FETCH_COLUMN);
$barangays = $pdo->query("SELECT DISTINCT barangay FROM user_application WHERE approved = 1 AND barangay IS NOT NULL ORDER BY barangay")->fetchAll(PDO::FETCH_COLUMN);

// Set default values if null
$totalAmount = $totalAmount ?: 0;
$uniqueBarangays = $uniqueBarangays ?: 0;
?>
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title"><i class="fas fa-check-circle mr-2"></i>Approved Applications</h3>
              </div>
              ...
              <div class="card-body">
                ...
                <div class="results-count">
                  ...
                  <strong>Total Approved Applications:</strong> <?= $totalBeneficiaries ?> records
                </div>
                ...
                <div class="table-responsive">
                  <table id="applicationTable" class="table table-hover display">
                    ...
                    <tbody>
                      <?php
                      $qry = $pdo->query("SELECT a.application_id, concat(a.b_fname, ' ', a.b_lname) as b_name,  s.scholarship_type, s.amount, a.barangay, a.school_name, a.year_level, a.school_year, a.semester 
                        FROM user_application as a 
                        INNER JOIN scholarship as s ON s.scholarship_id = a.scholarship_id 
                        WHERE a.approved = 1 
                        ORDER BY barangay;");
                      ...
                      ?>
                        <tr>
                          ...
                          <td class="font-weight-bold text-success">₱<?= number_format($row['amount'], 2) ?></td>
                        </tr>
                      <?php } ?>
                    </tbody>
...
$(document).ready(function(){
  ...
  var table = $('#applicationTable').DataTable({
    ...
    order: [[1, 'asc']],
    language: {
      search: "Search approved applications:",
      lengthMenu: "Show _MENU_ applications",
      info: "Showing _START_ to _END_ of _TOTAL_ approved applications",
      infoEmpty: "No approved applications available",
